const { isEqual, parseISO, subDays } = require('date-fns')
const { format, utcToZonedTime } = require('date-fns-tz')
const util = require('util')
const { byRange, daily, weekly, monthly, yearly } = require('./statistics')
...
exports.getOperations = util.promisify(async (db, query, callback) => {
  ...
  if (isEqual(dateFrom, dateTo)) {
    const operations = await daily(dateFrom, history)
    callback(null, operations)
    // return operations
  } else {
    const operations = await byRange(dateFrom, dateTo, history)
    callback(null, operations)
    // return operations
  }
})
