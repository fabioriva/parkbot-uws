...
class Log {
  constructor (s7log) {
    ...
    // this.date = new Date(s7log.date) // !!! save in UTC or timestamp
    // this.date_ = new Date(format(s7log.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"))
...
exports.getOperations_ = util.promisify(async (db, query, callback) => {
  try {
    const date = parseISO(query.dateString)
