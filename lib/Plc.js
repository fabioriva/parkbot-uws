...
const { updateExits, updateGarageScreens, updateOccupancy } = require('../models/screens')
...
class PLC extends EventEmitter {
  ...
  async data (def, obj) {
    try {
      ...
      await this.dss(def, obj, buffer) // DSS (New!)
    } catch (error) {
      this.error(error)
    } finally {
  ...
  async dss (def, obj, buffer) {
    try {
      if (obj.exits !== undefined) {
        updateExits(def.DB_DATA_INIT_EXITS, buffer, 4, obj.exits)
      }
      if (obj.screens !== undefined) {
        updateGarageScreens(def.DB_DATA_INIT_SCREENS, buffer, 10, obj.screens, obj.occupancy)
      }
      if (obj.occupancy !== undefined) {
        updateOccupancy(def.DB_DATA_INIT_OCCUPANCY, buffer, 6, obj.occupancy)
      }
      if (obj.exitScreen !== undefined) {
        obj.exitScreen.update(obj.exits, obj.queue)
      }
    } catch (error) {
      this.error(error)
    }
