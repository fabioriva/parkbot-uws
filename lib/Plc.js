...
const { updateGarageScreens } = require('../models/screens')
  ...
  async dss (def, obj, buffer) {
    try {
      if (obj.screens !== undefined) {
        updateGarageScreens(def.DB_DATA_INIT_SCREEN, buffer, 10, obj.screens)
      }
      if (obj.exitScreen !== undefined) {
        obj.exitScreen.update(def.DSS_EXIT_STEP_DONE, obj.exitGarages, obj.queue)
      }
    } catch (error) {
      this.error(error)
    }
