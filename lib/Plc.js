...
const { updateScreens } = require('../models/screen')
...
class PLC extends EventEmitter {
  ...
  async data (def, obj) {
    try {
      ...
      if (def.DB_DATA_INIT_SCREEN !== undefined) {
        updateScreens(def.DB_DATA_INIT_SCREEN, buffer, 10, obj.screens)
      }
    } catch (error) {
      this.error(error)
    } finally {
  ...
  // async dss (def, obj) {
  //   if (def.DSS_READ !== undefined) {
  //     try {
  //       const buffer = await this.read(def.DSS_READ)
  //       await updateScreens(0, buffer, 10, obj.screens)
