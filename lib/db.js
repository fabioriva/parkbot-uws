...
class Log {
  constructor (def, obj, s7log) {
    ...
    this.mode = this.mode(s7log, def)
    this.operation = this.operation(s7log, def)
  ...
  mode (log, def) {
    return {
      id: log.mode,
      info: def.MODES.find(item => item.id === log.mode).label
    }
  }
  ...
  operation (log, def) {
    return {
      id: log.operation,
      info: def.OP.find(item => item.id === log.operation).label
    }
  }
}
...
exports.insertLog = util.promisify((db, def, obj, s7log, callback) => {
  const log = new Log(def, obj, s7log)
