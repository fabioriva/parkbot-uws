...
function routes (app, db, def, obj, plc, options) {
  ...
  app.get(prefix + '/dashboard', async (res, req) => {
    ...
    const activity = await getRecentActivity_(db, 5)
  ...
  app.get(prefix + '/history/:id', async (res, req) => {
    ...
    const docs = await getHistoryLog_(db, req.getParameter(0))
    sendJson(res, docs)
  })
  ...
  app.post(prefix + '/system/operation', (res, req) => {
    readJson(
      res,
      async json => {
        // console.log('/system/operation', json)
        const card = parseInt(json.card)
  ...
  app.post(prefix + '/readArea', (res, req) => {
    readJson(
      ...
      async json => {
        ...
        console.log('readArea', response)
        sendJson(res, {
          type: response ? 'success' : 'error',
          info: response ? 'Read' : 'Read error!'
        })
      },
      () => {
        sendJson(res, {
  ...
  app.post(prefix + '/writeArea', (res, req) => {
    readJson(
      ...
      async json => {
        ...
        console.log('writeArea', response)
        sendJson(res, {
          type: response ? 'success' : 'error',
          info: response ? 'Written' : 'Write error!'
        })
