...
const { getappDateTime } = require('./utils7')
...
class S7Log {
  constructor (buffer) {
    ...
    this.date = getappDateTime(buffer.readInt16BE(20), buffer.readInt32BE(22))
    this.elapsed = buffer.readInt32BE(26)
    this.etx = buffer.readInt16BE(30)
  }
}

function log (db, plc, def, obj) {
  const { PORT, HOST } = def
  const server = net.createServer(function (socket) {
    ...
    socket.on('data', async function (data) {
      ...
      if (buffer.length === LOG_LEN) {
        ...
        const result = await insertLog(db, s7log, obj)
        console.info(result.ops[0])
        plc.app.publish('notification', JSON.stringify(result.ops.shift()))
      }
    })
  })
